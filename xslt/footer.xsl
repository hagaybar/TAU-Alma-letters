...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:param name="letter_language" select="'en'"/>
	<xsl:param name="lib_id" select="''"/>

	<!-- Try to find a text with matching label/lang/lib -->
	<xsl:variable name="matchLib"
		select="$additional_texts/text[@label = $label and @lang = $letter_language and @lib = $lib_id]"/>

	<!-- Fallback: matching label/lang with no lib attribute -->
	<xsl:variable name="matchGeneric"
		select="$additional_texts/text[@label = $label and @lang = $letter_language and not(@lib)]"/>
	...
	<xsl:choose>
		...
		<xsl:otherwise>
		<!-- No match: log warning, display nothing -->
