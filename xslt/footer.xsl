        ...
        <table>
            ...
            <tr>
                <xsl:for-each select="notification_data/organization_unit">
                    ...
                    <td align="center">
                        <xsl:value-of select="name" /> <xsl:value-of select="line1" /> <xsl:value-of select="line2" /> <xsl:value-of select="city" /> <xsl:value-of select="postal_code" /> <xsl:value-of select="country" /></td>
                </xsl:for-each>
            </tr>
        </table>
    </xsl:template>
        ...
        <tr>
            <td>
                <b>
					...
					<xsl:choose><xsl:when test="$letter_language = 'he'">
						דוא"ל:
						</xsl:when><xsl:otherwise>
						Email:
						</xsl:otherwise></xsl:choose><a href="mailto:{$rs_email}"><xsl:value-of select="$rs_email" /></a></b>
            </td>
        </tr>
    </xsl:template>
